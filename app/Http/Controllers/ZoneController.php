...
//     }
	...
	public function editCustomAd(Request $request)
    {
        ...
        if ($ad) {
			...
			$themes = $CUtil->getThemes($ad->id, 1);
			$states = $CUtil->getStates($ad->id, 1);
			$counties = $CUtil->getCounties($ad->id, 1);
			$platforms = $CUtil->getPlatforms($ad->id, 1);
			$os_targets = $CUtil->getOperatingSystems($ad->id, 1);
			$browser_targets = $CUtil->getBrowsers($ad->id, 1);
		...
		return view('zone_manage_customAd', [
                    ...
                    'browser_targets' => $browser_targets,
                    'platforms' => $platforms,
                    'keywords' => str_replace("|", ",", $ad->keywords),
		    		'counties' => $counties,
					'creatives' => $creatives
		]);
    }
	...
	if(!$this->checkBank()) return redirect('/addfunds');
        ...
        $creatives = AdCreative::where('ad_id', $request->id)->get();
        $media = Media::where([['status', 1],['user_id', $user->id]])->get();
        $links = Links::where([['status', 1],['user_id', $user->id]])->get();
        return view('new_creative', ['user' => $user, 'campaign' => $campaign, 'media' => $media, 'links' => $links, 'folders' => '']);
    }
