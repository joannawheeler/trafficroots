...
class PixelController extends Controller
{
    ...
    public function analyzeSite($site_handle)
    {
        $site = Site::where('site_handle', $site_handle)->first();
        if(sizeof($site)){
            $geo_table = '<table id="geo_table" width="100%" class="table table-striped table-border table-hover data-table"><thead><tr><th>Country</th><th>Impressions</th></tr></thead><tbody>';
            $sql = "SELECT geo, sum(impressions) as impressions FROM site_analysis WHERE site_handle = '".$site_handle."' GROUP BY geo ORDER BY impressions DESC";
            $result = DB::select($sql);
            if(sizeof($result)){
                foreach($result as $row){
                     $geo_table .= "\n<tr><td>".$row->geo."</td><td>".$row->impressions."</td></tr>";
                }
            }
            $geo_table .= "</tbody>\n</table>";

            $state_table = '<table id="state_table" width="100%" class="table table-striped table-border table-hover data-table"><thead><tr><th>State</th><th>Impressions</th></tr></thead><tbody>';
            ...
            $city_table = '<table id="city_table" width="100%" class="table table-striped table-border table-hover data-table"><thead><tr><th>City</th><th>Impressions</th></tr></thead><tbody>';
            $sql = "SELECT city, sum(impressions) as impressions FROM site_analysis WHERE site_handle = '".$site_handle."' AND geo = 'US' GROUP BY city ORDER BY impressions DESC";
            $result = DB::select($sql);
            if(sizeof($result)){
                foreach($result as $row){
                     $city_table .= "\n<tr><td>".$row->city."</td><td>".$row->impressions."</td></tr>";
                }
            }
            $city_table .= "</tbody>\n</table>";

            $device_table = '<table id="device_table" width="100%" class="table table-striped table-border table-hover data-table"><thead><tr><th>Country</th><th>Impressions</th></tr></thead><tbody>';
            ...
            if(sizeof($result)){
                foreach($result as $row){
                     $device_table .= "\n<tr><td>".$row->device."</td><td>".$row->impressions."</td></tr>";
                }
            }
            $device_table .= "</tbody>\n</table>";

            $browser_table = '<table id="browser_table" width="100%" class="table table-striped table-border table-hover data-table"><thead><tr><th>Browser</th><th>Impressions</th></tr></thead><tbody>';
            ...
            $os_table = '<table id="os_table" width="100%" class="table table-striped table-border table-hover data-table"><thead><tr><th>Operating System</th><th>Impressions</th></tr></thead><tbody>';
            $sql = "SELECT operating_systems.os, sum(site_analyis.impressions) as impressions 
                    FROM site_analysis
            ...
            return view('analysis', ['geo_table' => $geo_table, 'state_table' => $state_table, 'city_table' => $city_table, 'device_table' => $device_table, 'browser_table' => $browser_table, 'os_table' => $os_table]);
